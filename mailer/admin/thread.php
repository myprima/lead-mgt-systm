...
if ($op=='send' || $op=='cron') {
    ...
    if($contact_id) {
	$q=sqlquery("select form_id,name,0 as cust_id from forms where form_id='$x_form_id'");
    }
    else {
    	...
    	{
    		//$lstTable = "contacts";
			$q=sqlquery("select form_id,name,0 as cust_id from forms where form_id in ($camp_cat)");
    	}
		else 
		{
			//$lstTable = "customize";
			$q=sqlquery("select form_id,name,cust_id from customize_list where cust_id in ($camp_cat)");
		}
    }    
    $error_forms=array();	/* forms with no subscribe field */    
    while(list($form_id,$form,$cust_id)=sqlfetchrow($q)) {       
    ...
    {
    	list($email_field)=sqlget("
	    select name from customize_fields where active=1 and type=24 and
						    cust_id='$cust_id'");
    }
	...
	if($email_field) {
	    ...
	    if ($list_type == 1)
	    	...
	    	$q1=sqlquery("select name,type from customize_fields where cust_id='$cust_id' and
			  type not in (".join(',',array_merge($dead_fields,$secure_fields)).")");
	    ...
	    if ($list_type == 1)
	    	...
	    	list($subscribe_field)=sqlget("
			select name from customize_fields where type=6 and cust_id='$cust_id'");
...
while(true) {
    ...
    * (usually the case when we were resumed) */
    ...
    $body2=str_replace("%Euser%",$email,$body2);    
    $html2=str_replace("%Euser%",$email,$html2);    
    $url2=str_replace("%Euser%",$email,$url2);
    ...
    if($x_admin_header)
	    $m -> AddCustomHeader("X-Admin: $x_admin_header");
	if($x_address_line)
	    $m -> AddCustomHeader("X-Address-Line: $x_address_line");
    ...
    if ($op == 'test') {    	
    	if ($list_type == 1)    	
			...
			list($form)=sqlget("select name from customize_list where cust_id = '$f_form'");
    ...
    /**********************/        
    $body2		= str_replace("%First_Name%", $person['First_Name'], $body2);
    $html2		= str_replace("%First_Name%", $person['First_Name'], $html2);
    /*******************/
    $m -> Subject	= $subject2;
	...
	if($url2) {
     ...
     if ($cust_id)
		...
		where cust_id='$cust_id' and type=8");	    
	else 
		list($has_password)=sqlget("
		select count(*) from form_fields
		where form_id='$form_id' and type=8");
		
    $profile_text1=preg_replace("/<a href=%profile%>(.*?)<\/a>/i","\\1\nhttp://$server_name2/users/members.php?profile2=1&login=$email&form_id=$form_id&noerror=1&p=".base64_encode($password)."&check2_x=1&cust_id=$cust_id",$profile_text);
    $profile_text2=str_replace('%profile%',"http://$server_name2/users/members.php?profile2=1&u=$email&form_id=$form_id&noerror=1&p=".base64_encode($password)."&check2_x=1&cust_id=$cust_id",$profile_text);
    $profile_text3=str_replace('click here: <a href=%profile%></a>',"<a href='http://$server_name2/users/members.php?profile2=1&u=$email&form_id=$form_id&noerror=1&p=".base64_encode($password)."&check2_x=1&cust_id=$cust_id'>click here</a>",$profile_text);
    

    $unsub_text1=preg_replace("/<a href=%unsub%>(.*?)<\/a>/i","\\1\nhttp://$server_name2/users/unsubscribe.php?email=$email&form_id=$form_id&cust_id=$cust_id",$unsub_text);
    $unsub_text2=str_replace('%unsub%',"http://$server_name2/users/unsubscribe.php?email=$email&form_id=$form_id&cust_id=$cust_id",$unsub_text);
    $unsub_text3=str_replace('click here: <a href=%unsub%></a>',"<a href='http://$server_name2/users/unsubscribe.php?email=$email&form_id=$form_id&cust_id=$cust_id'>click here</a>",$unsub_text);

    $share_text1=preg_replace("/<a href=%share%>(.*?)<\/a>/i","\\1\nhttp://$server_name2/users/share_friends.php?contact_id=$contact_id&form_id=$form_id&email_id=$id&stat_id=$stat_id&cust_id=$cust_id",$share_text);
    $share_text2=str_replace('%share%',"http://$server_name2/users/share_friends.php?contact_id=$contact_id&form_id=$form_id&email_id=$id&stat_id=$stat_id&cust_id=$cust_id",$share_text);
    $share_text3=str_replace('click here: <a href=%share%></a>',"<a href='http://$server_name2/users/share_friends.php?contact_id=$contact_id&form_id=$form_id&email_id=$id&stat_id=$stat_id&cust_id=$cust_id'>click here</a>",$share_text);
...
#echo "<pre>$body2</pre>";
    ...
    if($body2 && (($email_format & 2) || (!$html2 && !$url2))) {
    	...
    	$body2=str_replace('%profile%',"http://$server_name2/users/members.php?profile2=1&u=$email&form_id=$form_id&noerror=1&p=".base64_encode($password)."&check2_x=1&cust_id=$cust_id",$body2);    	    	
    	$body2=str_replace('%unsub%',"http://$server_name2/users/unsubscribe.php?email=$email&form_id=$form_id&cust_id=$cust_id",$body2);
    	$body2=str_replace('%share%',"http://$server_name2/users/share_friends.php?contact_id=$contact_id&form_id=$form_id&email_id=$id&stat_id=$stat_id&cust_id=$cust_id",$body2);
    ...
    if($html2 && (($email_format & 1) || (!$body2 && !$url2))) {
    	...
    	$html2=str_replace('%profile%',"http://$server_name2/users/members.php?profile2=1&u=$email&form_id=$form_id&noerror=1&p=".base64_encode($password)."&check2_x=1&cust_id=$cust_id",$html2);    	    	
    	$html2=str_replace('%unsub%',"http://$server_name2/users/unsubscribe.php?email=$email&form_id=$form_id&cust_id=$cust_id",$html2);
    	$html2=str_replace('%share%',"http://$server_name2/users/share_friends.php?contact_id=$contact_id&form_id=$form_id&email_id=$id&stat_id=$stat_id&cust_id=$cust_id",$html2);
    	
		$html2=eregi_replace("</body>","<br>$unsub_text2</body>",$html2);
    	if($allow_profile && $has_password){
			$html2=eregi_replace("</body>","<br>$profile_text2</body>",$html2);
	    }
	    if($share_email){
	    	$html2=eregi_replace("</body>","<br>$share_text2</body>",$html2);
	    }
	    if($monitor_reads){
    ...
    if($url2 && (($email_format & 1) || (!$body2 && !$html2))) {
        $url2=eregi_replace("</body>","<br>$unsub_text3</body>",$url2);
        if($allow_profile && $has_password)
	    	$url2=eregi_replace("</body>","<br>$profile_text3</body>",$url2);
		if($share_email)
	    	$url2=eregi_replace("</body>","<br>$share_text3</body>",$url2);
		if($monitor_reads)
