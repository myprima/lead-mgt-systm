...
//			print_r($field_types);
			...
			for($m=0; $m<count($i_fields); $m++)			
			{
				if ($field_types[$i_fields[$m]]==11)
				{
					$i_vals[$m]=str_replace("'","",$i_vals[$m]);
					$dt = explode("/",$i_vals[$m]);
					$i_vals[$m] = "'".$dt[2]."-".$dt[0]."-".$dt[1]."'";										
				}
			}			

		    $q=sqlquery("INSERT INTO contacts_$form2 (`".join('`,`',$i_fields)."`,approved,added,ip)
			VALUES (".join(",",$i_vals).",".(($strict_require_user || $send_confirm) ? 3 : 2).",now(),'$REMOTE_ADDR')");

		    if(!sql_error()) {
			$contact_id=sqlinsid($q);
			sqlquery("insert into responder_subscribes (responder_id,
				    contact_id,form_id,added)
				  select responder_id,'$contact_id','$form_id',now()
				  from forms_responders where form_id='$form_id'");
			/////////////////////////////////
			if($strict_require_user || $send_confirm) {
			    /**
				...
				$details =	array(
				    'email_list_name' => $form_name,
				    'approve_link' => "http://$server_name2/users/validate.php?id=$hash&form_id=$form_id",
				    'remove_database_link' => "http://$server_name2/users/unsubscribe.php?email=$email&form_id=$form_id&del_user=1",
				    'email_domain'=> $email_domain
				);
				notify_email($email, 'Confirm User Email', $details, false);
			    }
			}				
			/////////////////////////////////						
...
#		    where ".join(' and ',array_diff($u_fields,$sec_fld));
		if($contact_id) {
		    sqlquery("delete from contacts_$form2 where contact_id='$contact_id'");
