<?php
    ...
    if(!$helpfile) 
    {
	    if(($tmphelp=get_help(basename($_SERVER[PHP_SELF]),
			...
			$helpfile=$tmphelp;
			if(empty($helpfile))
				$helpfile = "http://upload.hostcontroladmin.com/userguides/mailer/";
    }
...
<?php
    ...
    if(!$has_admin_header)
    	echo "<img src=../images/images.jpg border=0>";    
?>
	</a></td>
    <td height=40
<?php
...
<?php
	...
	if($order_software)
	    echo "<A href=$order_software target=_new><u>Order Additional Software</u></a>";
	if($feedback_link || $order_software)
	    echo "</b>";
    }
?>
...
if($show_help && $helpfile && strpos($PHP_SELF,'forget.php')===false)
	$tmp_help='User Guide';
			
if(strpos($PHP_SELF,'index.php')!==false)
	echo "<div align=right><br><a href=$helpfile target='_blank'>$tmp_help</a> | <a href=logout.php>Log Off</a>&nbsp;&nbsp;&nbsp;<br></div>";
else 
	echo "<div align=right><br><a href=index.php>Main Menu</a> | <a href=$helpfile target='_blank'>$tmp_help</a> | <a href=logout.php>Log Off</a>&nbsp;&nbsp;&nbsp;<br></div>";	
