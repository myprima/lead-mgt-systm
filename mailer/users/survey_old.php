...
if($x_table1!='#007cb8') {
    BeginForm(2,0,'','',($x_table1?$x_table1:'#007cb8'));
}
else {
...
display_fields('survey','','',"active=1 and survey_id='$survey_id'",'',array('object_id'=>$survey_id));
...
if($check=='Submit Survey' && !$bad_form) {
...
#	list($from)=sqlget("select noreply from config");
	...
	$q=sqlquery("
	    ...
	    from user_group,user,contacts_intgroups_$form2,grants
	    where user_group.user_id=user.user_id and
		group_id in (3) and email<>'' and
		notify=1 and '$notrack'<>1 and
	        contacts_intgroups_$form2.category_id=grants.object_id and
		grants.user_id=user.user_id and right_id in (3,4,5)");
	while(list($email)=sqlfetchrow($q) && $body && $user_id) {
	    list($server_name3,$bullshit)=explode('/',$server_name2);
