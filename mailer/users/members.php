...
if($sid) {
	if ($save_form)
		...
		where session.hash='$sid' and user.user_id=session.user_id and
	    	    user.user_id=user_group.user_id and user.form_id=$save_form and 
...
if($login) {
	if ($cust_id)
	{
		list($cname) = sqlget("select name from customize_list where cust_id='$cust_id'");
		$cname = castrate($cname);
		list($email) = sqlget("select name from `customize_fields` where cust_id='$cust_id' and type=24");
		$email = castrate($email);
		list($user_id)=sqlget("select user_id from customize_$cname where $email = '$login'");
	}	
	elseif ($save_form)
		list($user_id,$form_id)=sqlget("
...
if(($user_id && $login) || $cust_id) {
    if($check2_x || $profile) {
    if ($cust_id)	
		$redirect="profile.php?cust_id=$cust_id";
	else 
...
#    header("Authorization: Basic ".base64_encode("$login:$password"));
    ...
    $error="<font color=red>Login incorrect</font><br>";
}
  ...
  <TD width=521 align=middle><BR>
    <TABLE width=481 border=0 align=center bgcolor=#f4f4f4>
	  ...
	  <STRONG>$msg_members[1] </STRONG></FONT></TD>
          <TD width=75%>
          <INPUT size=40 name=login value='$login'>
          </TD>
        </TR>
        <TR>
