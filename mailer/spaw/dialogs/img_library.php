...
if (isset($HTTP_POST_FILES['img_file']['size']) && $HTTP_POST_FILES['img_file']['size']>0)
{
  ...
  {
    $preview = $spaw_base_url./*$imglib*/'/img/'.$img;
  }
}
// delete
...
<head>
  ...
  <!--
    function selectClick()
    {
      ...
      {
        window.returnValue = '<?php echo $spaw_base_url."img/"?>'+ document.getElementById('imglist').options[document.getElementById('imglist').selectedIndex].value;
        window.close();
        <?php
...
<tr>
  ...
  <select name="imglist" id="imglist" size="15" class="input" style="width: 150px;"
    onchange="if (this.selectedIndex &gt;=0) document.getElementById('imgpreview').src = '<?php echo $spaw_base_url.'/img/'/*$imglib*/?>' + this.options[this.selectedIndex].value;" ondblclick="selectClick();">
  <?php
